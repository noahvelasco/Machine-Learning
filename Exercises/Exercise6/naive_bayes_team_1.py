...
class naive_bayes():
    ...
    def predict(self,x_test, y_test):
        ...
        for i in range(len(x_test)):
            prob_ones = np.prod(self.p_att_given_class[:,x_test[i] == 1], axis=1)
            prob_zeros = np.prod((1-self.p_att_given_class[:,x_test[i] == 0]), axis=1)
            pred[i] = np.argmax(self.p_class * prob_ones * prob_zeros)
        return pred
...
Elapsed_time training:  1.401542 secs
Elapsed_time testing: 0.470353 secs
Accuracy: 0.835857    
'''    